...
struct ASpT : Matrix_Format
{
	...
	ASpT(INT_T * row_ptr_in, INT_T * col_ind, ValueTypeReference * values, long m, long n, long nnz) : Matrix_Format(m, n, nnz)
	{
...
void
compute_csr(ASpT * restrict csr, long K, ValueType * A, ValueType * B, ValueType * C)
{
	#pragma omp parallel
	{
		int tnum = omp_get_thread_num();
