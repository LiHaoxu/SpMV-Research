		...
		unsigned char * buf_stealable, __attribute__((unused)) unsigned char * buf_protected, long num_vals, long * num_vals_out, long * size_stealable_out, long * size_protected_out)
{
	...
	*num_vals_out = num_vals;
	...
	for (k=0;k<num_vals;k++)
	{
		...
		val.d = window_buf[k];   // Compare negatives as unsigned integers => compare by absolute values.
		val.d = val.d * -1;      // Has to work with any float type.
		compare_data_packed[k].u64[1] = val.u;
		compare_data_packed[k].u32[1] = cols_buf[k];
		compare_data_packed[k].u32[0] = k;
	}
	// quicksort_packed(rev_permutation, num_vals, compare_data_packed, tdk->qsort_partitions);
	...
	for (k=0;k<num_vals;k++)
	{
		rev_permutation[k] = compare_data_packed[k].u32[0];
	}
	...
	bucketsort_stable_serial(window_rf, num_vals, num_buckets, NULL, permutation_buf, offsets_rf, NULL);

	for (k=0;k<num_vals;k++)
	{
		rev_permutation_buf[permutation_buf[k]] = rev_permutation[k];
		window_buf[permutation_buf[k]] = window[k];
		window_rf_buf[permutation_buf[k]] = window_rf[k];
	}
