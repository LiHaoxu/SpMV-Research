...
// {
	...
	// int ca=cols[a], cb=cols[b];
	// int col_dist = 64;
	// int ca_div = ca / col_dist, cb_div = cb / col_dist;
	// double va=vals[a], vb=vals[b];
	// int ret;
	// ret = (ca_div > cb_div) ? 1 : (ca_div < cb_div) ? -1 : 0;
	// if (ret == 0)
	// {
		// if (va*vb <= 0)
			// ret = (va > vb) ? 1 : (va < vb) ? -1 : 0;
		...
		)
{
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique_multiple;i+=vec_len_pd)
		{
			...
			// vec_i_t sign = diff.u & (1ULL << 63ULL);
			vec_i_t sign = vec_and_si(diff.u, vec_set1_epi64(1ULL << 63ULL));
			// sign = sign >> shift;
			...
			// diff.u = diff.u >> shift;
			diff.u = vec_srlv_epi64(diff.u, shift);
