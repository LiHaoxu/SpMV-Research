...
extern "C"{
	...
	#include "div_gather_coords.h"
	// #include "div_gather_coords_avx2.h"
#ifdef __cplusplus
}
#endif
...
extern "C"{
	...
	#define QUICKSORT_GEN_SUFFIX  _sym
...
extern "C"{
	...
	#define QUICKSORT_GEN_TYPE_2  int
	#define QUICKSORT_GEN_TYPE_3  struct cmp_data
	#define QUICKSORT_GEN_SUFFIX  _i_i_d
	#include "sort/quicksort/quicksort_gen.c"
#ifdef __cplusplus
}
#endif
...
static inline
long
compress_kernel_sort_diff_base(unsigned char * buf,
		long num_vals,
		double * window_buf,
		...
		)
{
	...
	uint64_t len_bits = 0;
	uint64_t len = 0;
	uint64_t row = 0, row_min = 0, row_max = 0, row_diff = 0, row_diff_max = 0, col = 0, col_min = 0, col_max = 0, col_diff = 0, col_diff_max = 0;   // row_max is the last row (i.e., inclusive: num_rows = row_max + 1 - row_min)
	...
	{
		if (row_bits > 8)
			error("row_bits > 8 (%ld), row_min=%ld, row_max=%ld", row_bits, row_min, row_max);
	}
	...
	if (symmetric)
		quicksort_sym(rev_permutation, num_vals, &compare_data, t_qsort_partitions[tnum]);
	else
	...
	for (k=0;k<num_vals;k++)
	{
		if (rows[k] > row_max)
			error("row index");
		if (rev_permutation[k] >= num_vals)
			error("rev_permutation[k] >= num_vals");
...
compress_kernel_sort_diff(INT_T * row_ptr, INT_T * ja, ValueType * vals, long i_s, long i_t_s, long i_t_e, long j_s, unsigned char * buf, long num_vals_suggested, long * num_vals_out)
{
	...
	/* Store the sizes of the two sub-packets. */
	uint32_t * data_intro_packet_bytes_ptr = (typeof(data_intro_packet_bytes_ptr)) &data[data_bytes];
	*data_intro_packet_bytes_ptr = 0;
	data_bytes += sizeof(uint32_t);
	uint32_t * data_intro_packet_sym_bytes_ptr = (typeof(data_intro_packet_sym_bytes_ptr)) &data[data_bytes];
	*data_intro_packet_sym_bytes_ptr = 0;
	data_bytes += sizeof(uint32_t);

	long packet_bytes = 0, packet_sym_bytes = 0;

	if (num_vals > 0)
	{
		packet_bytes = compress_kernel_sort_diff_base(&data[data_bytes],
			num_vals,
			window_buf,
			rows_buf,
			cols_buf,
		...
		*data_intro_packet_bytes_ptr = packet_bytes;
		data_bytes += packet_bytes;
	}

	if (num_vals_sym > 0)
	{
		packet_sym_bytes = compress_kernel_sort_diff_base(&data[data_bytes],
			num_vals_sym,
			window_sym,
			rows_sym,
			cols_sym,
		...
		*data_intro_packet_sym_bytes_ptr = packet_sym_bytes;
		data_bytes += packet_sym_bytes;
	}
		...
		__attribute__((unused)) ValueType * sum_ptr, __attribute__((unused)) uint64_t * row_rel_prev_ptr, __attribute__((unused)) uint64_t * col_rel_prev_ptr, __attribute__((unused)) uint64_t * row_prev_ptr, __attribute__((unused)) uint64_t * col_prev_ptr)
{
	...
	// {
		// y_rel[*row_rel_prev_ptr] += *sum_ptr;
		// *row_rel_prev_ptr = row_rel;
		// *sum_ptr = 0;
	// }
	// *sum_ptr += val * x_rel[col_rel];
		...
		__attribute__((unused)) vec_d_t * sum_ptr, __attribute__((unused)) vec_i_t * row_rel_prev_ptr, __attribute__((unused)) vec_i_t * col_rel_prev_ptr, __attribute__((unused)) vec_i_t * row_prev_ptr, __attribute__((unused)) vec_i_t * col_prev_ptr)
{
	// for (long iter=0;iter<vec_len_pd;iter++)
	// {
		// if (row_rel[iter] != (*row_rel_prev_ptr)[iter])
		// {
			// y_rel[(*row_rel_prev_ptr)[iter]] += (*sum_ptr)[iter];
			// (*row_rel_prev_ptr)[iter] = row_rel[iter];
			// (*sum_ptr)[iter] = 0;
		// }
		// (*sum_ptr)[iter] += val[iter] * x_rel[col_rel[iter]];
	// }
	for (long iter=0;iter<vec_len_pd;iter++)
		...
		__attribute__((unused)) vec_d_t * sum_ptr, __attribute__((unused)) vec_i_t * row_rel_prev_ptr, __attribute__((unused)) vec_i_t * col_rel_prev_ptr, __attribute__((unused)) vec_i_t * row_prev_ptr, __attribute__((unused)) vec_i_t * col_prev_ptr)
{
	for (long iter=0;iter<vec_len_pd;iter++)
	{
		if (col[iter] != (*col_prev_ptr)[iter])
		{
			y[(*col_prev_ptr)[iter]] += (*sum_ptr)[iter];
			(*col_prev_ptr)[iter] = col[iter];
			(*sum_ptr)[iter] = 0;
		}
		(*sum_ptr)[iter] += val[iter] * x[row[iter]];
		y[row[iter]] += val[iter] * x[col[iter]];
	}
}
...
decompress_and_compute_kernel_sort_diff_base(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate,
		...
		void (* mult_add)(ValueType * x_rel, ValueType * y_rel, ValueType * x, ValueType * y, ValueType val, uint64_t row_rel, uint64_t col_rel, uint64_t row, uint64_t col,
		...
		)
{
	...
	vec_d_t sum = vec_set1_pd(0);
	vec_i_t row_rel_prev, col_rel_prev;
	vec_i_t row_prev, col_prev;
	gather_coords_v(0, data_coords, coords_bytes, row_bits, col_bits, &row_rel_prev, &col_rel_prev);
	row_prev = vec_add_epi64(row_rel_prev, vec_set1_epi64(row_min));
	col_prev = vec_add_epi64(col_rel_prev, vec_set1_epi64(col_min));
	for (l=0;l<num_rfs;l++)
	{
		union {
			vec_d_t d;
		...
		for (i=0;i<num_rf_vals_unique_multiple;i+=vec_len_pd)
		{
			...
			for (j=0;j<rf;j++)
			{
				...
				vec_i_t row, col;
				row = vec_add_epi64(row_rel, vec_set1_epi64(row_min));
				col = vec_add_epi64(col_rel, vec_set1_epi64(col_min));

				if (validate)
					vec_storeu_pd(&window[k], val.d);
				else
				{
					mult_add_v(x_rel, y_rel, x, y, val.d, row_rel, col_rel, row, col, &sum, &row_rel_prev, &col_rel_prev, &row_prev, &col_prev);
				}
			}
		}
		...
		for (i=num_rf_vals_unique_multiple;i<num_rf_vals_unique;i++)
		{
			...
			for (j=0;j<rf_multiple;j+=vec_len_pd)
			{
				...
				{
					mult_add_v(x_rel, y_rel, x, y, val_buf, row_rel, col_rel, row, col, &sum, &row_rel_prev, &col_rel_prev, &row_prev, &col_prev);
				}
			}
			...
			for (j=rf_multiple;j<rf;j++)
			{
				...
				{
					mult_add(x_rel, y_rel, x, y, val.d[lane_id], row_rel, col_rel, row, col, &sum[lane_id], (uint64_t *) &row_rel_prev[lane_id], (uint64_t *) &col_rel_prev[lane_id], (uint64_t *) &row_prev[lane_id], (uint64_t *) &col_prev[lane_id]);
				}
			}
		}
	...
	if (!validate)
	{
		mult_add_cleanup(num_vals, y_rel, y, &sum, &row_rel_prev, &row_prev, &col_rel_prev, &col_prev);
	}
		...
		)
{
	...
	// if (row_bits == 8)
	{
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r1_c1, gather_coords_sparse_r1_c1_v, mult_add, mult_add_v, mult_add_cleanup);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r1_c2, gather_coords_sparse_r1_c2_v, mult_add, mult_add_v, mult_add_cleanup);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r1_c3, gather_coords_sparse_r1_c3_v, mult_add, mult_add_v, mult_add_cleanup);
			case 32:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r1_c4, gather_coords_sparse_r1_c4_v, mult_add, mult_add_v, mult_add_cleanup);
		}
	}
	...
	if (row_bits == 0)
	{
		// return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0, gather_coords_sparse_r0_v, mult_add, mult_add_v, mult_add_cleanup);
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c1, gather_coords_sparse_r0_c1_v, mult_add, mult_add_v, mult_add_cleanup);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c2, gather_coords_sparse_r0_c2_v, mult_add, mult_add_v, mult_add_cleanup);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c3, gather_coords_sparse_r0_c3_v, mult_add, mult_add_v, mult_add_cleanup);
			case 32:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c4, gather_coords_sparse_r0_c4_v, mult_add, mult_add_v, mult_add_cleanup);
		}
	}

	switch (coords_bytes) {
		case 1:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_dense_1, gather_coords_dense_1_v, mult_add, mult_add_v, mult_add_cleanup);
		case 2:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_dense_2, gather_coords_dense_2_v, mult_add, mult_add_v, mult_add_cleanup);
		case 3:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_dense_3, gather_coords_dense_3_v, mult_add, mult_add_v, mult_add_cleanup);
		case 4:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_dense_4, gather_coords_dense_4_v, mult_add, mult_add_v, mult_add_cleanup);
	}

	return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_dense, gather_coords_dense_v, mult_add, mult_add_v, mult_add_cleanup);
...
decompress_and_compute_kernel_sort_diff(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, __attribute__((unused)) long i_t_s, __attribute__((unused)) long i_t_e)
{
	...
	uint64_t data_intro_bytes = 0;
	unsigned char * data;
	uint64_t packet_bytes = 0, packet_sym_bytes = 0;

	uint32_t data_intro_packet_bytes = *((uint32_t *) &data_intro[data_intro_bytes]);
	data_intro_bytes += sizeof(uint32_t);
	uint32_t data_intro_packet_sym_bytes = *((uint32_t *) &data_intro[data_intro_bytes]);
	data_intro_bytes += sizeof(uint32_t);

	data = &data_intro[data_intro_bytes];

	if (data_intro_packet_bytes > 0)
	{
		packet_bytes = decompress_and_compute_kernel_sort_diff_select(data, x, y, NULL, 0, mult_add_serial, mult_add_vector, mult_add_cleanup);
	}
	data += packet_bytes;

	if (data_intro_packet_sym_bytes > 0)
	{
		packet_sym_bytes = decompress_and_compute_kernel_sort_diff_select(data, x, y, NULL, 0, mult_add_serial_sym, mult_add_vector_sym, mult_add_cleanup_sym);
	}
	data += packet_sym_bytes;

	return data_intro_bytes + packet_bytes + packet_sym_bytes;
}
...
decompress_kernel_sort_diff(ValueType * vals, unsigned char * restrict buf, __attribute__((unused)) long i_t_s, __attribute__((unused)) long i_t_e)
{
	...
	if (data_intro_packet_bytes > 0)
	{
		packet_bytes = decompress_and_compute_kernel_sort_diff_select(data, NULL, NULL, &num_vals, 1, mult_add_serial, mult_add_vector, mult_add_cleanup);
	}
	data += packet_bytes;

	if (data_intro_packet_sym_bytes > 0)
	{
		packet_sym_bytes = decompress_and_compute_kernel_sort_diff_select(data, NULL, NULL, &num_vals, 1, mult_add_serial_sym, mult_add_vector_sym, mult_add_cleanup_sym);
	}
	data += packet_sym_bytes;
