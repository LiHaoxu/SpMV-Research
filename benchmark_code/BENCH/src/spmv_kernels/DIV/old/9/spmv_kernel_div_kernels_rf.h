...
struct thread_data_kernel {
	...
	struct cmp_data_packed_t * compare_data_packed;
	struct cmp_data_packed_t * compare_data_packed_buf;
};
...
// #endif
	...
	// #define QUICKSORT_GEN_TYPE_3  struct cmp_data_packed_t
	// #define QUICKSORT_GEN_SUFFIX  _packed
...
// static inline
// int
// quicksort_cmp(int a, int b, struct cmp_data_packed_t * aux)
// {
...
#endif
	#include "sort/quicksort/quicksort_gen_undef.h"
	#define QUICKSORT_GEN_TYPE_1  struct cmp_data_packed_t
	#define QUICKSORT_GEN_TYPE_2  int
	#define QUICKSORT_GEN_TYPE_3  void
	#define QUICKSORT_GEN_SUFFIX  _packed_perm
	#include "sort/quicksort/quicksort_gen.c"
#ifdef __cplusplus
}
#endif
...
// Quicksort_blocked
#include "sort/quicksort_blocked/quicksort_blocked_gen_undef.h"
#define QUICKSORT_BLOCKED_GEN_TYPE_1  struct cmp_data_packed_t
#define QUICKSORT_BLOCKED_GEN_TYPE_2  int
#define QUICKSORT_BLOCKED_GEN_TYPE_3  void
...
long permutation_table_i64_n;
uint64_t * permutation_table_i64;
long permutation_table_i64_as_i32_n;
uint32_t * permutation_table_i64_as_i32;
...
create_permutation_table()
{
	...
	uint64_t mask;
	long i, j, k, l, r;
	permutation_table_i64_n = VEC_LEN * (1ULL << VEC_LEN);
	permutation_table_i64 = (typeof(permutation_table_i64)) malloc(permutation_table_i64_n * sizeof(*permutation_table_i64));
	quicksort_permutation_table_i64 = (typeof(quicksort_permutation_table_i64)) malloc(permutation_table_i64_n * sizeof(*quicksort_permutation_table_i64));
	permutation_table_i64_as_i32_n = 2 * VEC_LEN * (1ULL << VEC_LEN);
	permutation_table_i64_as_i32 = (typeof(permutation_table_i64_as_i32)) malloc(permutation_table_i64_as_i32_n * sizeof(*permutation_table_i64_as_i32));
	for (i=0;i<1LL << VEC_LEN;i++)
	{
		...
		for (k=0;k<VEC_LEN;k++)
		{
			...
			{
				permutation[r] = k;
				permutation_table_i64[j + r] = k;
				permutation_table_i64_as_i32[2*j + 2*r] = 2*k;   // Little-endian
				permutation_table_i64_as_i32[2*j + 2*r+1] = 2*k+1;
				r--;
			}
			else
			{
				permutation[l] = k;
				permutation_table_i64[j + l] = k;
				permutation_table_i64_as_i32[2*j + 2*l] = 2*k;
				permutation_table_i64_as_i32[2*j + 2*l+1] = 2*k+1;
				l++;
			}
			mask <<= 1;
		}
		...
		struct cmp_data_packed_t * buf_out, long * i_buf_l_ptr, long * i_buf_r_ptr)
{
	...
	// vec_t(i32, 8) perm = vec_loadu(i32, 8, (vec_t(i32, 8) *) &permutation_table_i64_as_i32[2*mask * VEC_LEN]);
	// val0 = vec_i64_4(vec_permutexvar(i32, 8, val0, perm).v);
	// val1 = vec_i64_4(vec_permutexvar(i32, 8, val1, perm).v);
...
compress_init_div(__attribute__((unused)) ValueTypeReference * vals, __attribute__((unused)) const long num_vals, long packet_size)
{
	...
	#pragma omp parallel
	{
		...
		__aligned_alloc(tdk->compare_data_packed, window_size);
		__aligned_alloc(tdk->compare_data_packed_buf, window_size);
		for (i=0;i<VEC_LEN;i++)
		{
		...
		unsigned char * buf_stealable, [[gnu::unused]] unsigned char * buf_protected, long num_vals, long * num_vals_out, long * size_stealable_out, long * size_protected_out)
{
	...
	struct cmp_data_packed_t * compare_data_packed = tdk->compare_data_packed;
	struct cmp_data_packed_t * compare_data_packed_buf = tdk->compare_data_packed_buf;
	...
	*num_vals_out = num_vals;
	...
	for (k=0,i=row_min,j=j_s; k<num_vals; k++,j++)
	{
		...
		while (j >= row_ptr[i+1])
			i++;
		if (tolerance > 0)
			val.d = reduce_precision(vals[k], tolerance);
		else
			val.d = vals[k];
	...
	{
		compare_data_packed_buf[permutation[k]] = compare_data_packed[k];
		window_rf_buf[permutation[k]] = window_rf[k];
	}
	macros_swap(&compare_data_packed, &compare_data_packed_buf);
	...
	unsigned char * data_intro = buf_stealable;
	uint64_t data_intro_bytes = sizeof(*header);

	header = (typeof(header)) buf_stealable;
	...
	*size_stealable_out = data_intro_bytes + data_rfs_bytes + data_num_unique_vals_per_rf_bytes + data_coords_bytes + data_val_lens_bytes + data_val_lanes_bytes;
