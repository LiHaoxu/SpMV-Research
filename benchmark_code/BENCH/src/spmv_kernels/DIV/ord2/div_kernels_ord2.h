...
compress_kernel_sort_diff(INT_T * row_ptr, INT_T * ja, ValueType * vals, long i_s, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	*((uint32_t *) &data_intro[data_intro_bytes]) = col_min;
	...
	for (i=0;i<num_vals;i++)
	{
		...
		uint64_t max_sb = __builtin_clrsbl(max_diff_ord2);
		for (long j=0;j<16;j++)
		{
			uint64_t dd = diff.u - diff_prev_buf[lane_id][j].u;
			uint64_t sb = __builtin_clrsbl(dd);
			if (sb > max_sb)
			{
				max_pos = j;
				max_diff_ord2 = dd;
		...
		if (i_s == 0 && lane_id == 0)
			printf("diff=%064lb diff_ord2=%064lb diff_ord2_best16=%064lb %3d %3d %3d , %3d %3d\n", diff.u, diff_ord2.u, max_diff_ord2, __builtin_clrsbl(diff_ord2.u), __builtin_clrsbl(diff.u), (int) max_sb,
					__builtin_clrsbl(diff_ord2.u) - __builtin_clrsbl(diff.u), ((int) max_sb) - __builtin_clrsbl(diff.u));
