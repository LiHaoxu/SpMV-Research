	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique;i++)
		{
			...
			else
			{
				...
				len = (len_bits + 7ULL) >> 3ULL;
				if (len > 8)
					error("len > 8: len=%ld, clz=%ld, ctz=%ld", len, leading_zeros, trailing_zeros);

				diff_ord2 >>= trailing_zeros;
				bytestream_write_unsafe_cast(&Bs[lane_id], (uint64_t) diff_ord2, len);   // Bytestream works with 'uint64_t' buffer (so we cast diff), and the actual size in bytes 'len'.
		...
		)
{
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique_multiple;i+=VEC_LEN)
		{
			...
			// diff = diff ^ diff_ord2;
			diff = vec_xor(VTI, VEC_LEN, diff, diff_ord2);
			// val = val + diff;
