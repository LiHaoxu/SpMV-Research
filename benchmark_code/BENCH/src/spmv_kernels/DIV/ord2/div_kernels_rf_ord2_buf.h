	...
	*((uint32_t *) &data_intro[data_intro_bytes]) = num_rfs;
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique;i++)
		{
			...
			long cur_buf_pos = (i / vec_len_pd) % 16;
			diff_prev_buf[cur_buf_pos][lane_id].u = 0;
			uint64_t best_buf_pos = cur_buf_pos;
			uint64_t best_diff_ord2 = diff.u - diff_prev_buf[best_buf_pos][lane_id].u;
			uint64_t best_clz = __builtin_clzl(best_diff_ord2);
			for (long j=0;j<16;j++)
			{
				uint64_t dd = diff.u - diff_prev_buf[j][lane_id].u;
				uint64_t sb = __builtin_clzl(dd);
				if (((int64_t) dd > 0) && sb > best_clz)
				{
					best_buf_pos = j;
					best_diff_ord2 = dd;
					best_clz = sb;
			...
			// if (i_s == 0 && lane_id == 0)
				// printf("diff=%064lb diff_ord2=%064lb diff_ord2_best16=%064lb %3d %3d %3d , %3d %3d , tz=%3d\n", diff.u, diff_ord2.u, best_diff_ord2, __builtin_clzl(diff_ord2.u), __builtin_clzl(diff.u), (int) best_clz,
						// __builtin_clzl(diff_ord2.u) - __builtin_clzl(diff.u), ((int) best_clz) - __builtin_clzl(diff.u), __builtin_ctzl(diff.u));

			diff_ord2.u = best_diff_ord2;
			...
			else
			{
				 ...
				 *
				 */
				uint64_t buf_diff_prev;
				uint64_t buf_diff = diff.u;
				uint64_t leading_zeros = 0;
				leading_zeros = __builtin_clzl(diff_ord2.u);

				len_bits = 64ULL - leading_zeros;
				...
				uint64_t pos = (len >> 4ULL);
				if (pos != best_buf_pos)
					error("pos: %ld %ld %ld", len, pos, best_buf_pos);
				best_buf_pos = len >> 4ULL;
				len &= 15ULL;
				uint64_t mask = (len == 8) ? -1ULL : (1ULL << len_bits) - 1ULL;
				diff_ord2.u &= mask;
				buf_diff_prev = diff_prev_buf[best_buf_pos][lane_id].u;
				if (buf_diff_prev + diff_ord2.u != buf_diff)
					error("diff_ord2.u != buf_diff , len=%ld", len);
			}

			diff_prev_buf[cur_buf_pos][lane_id].u = diff.u;
		}
		...
		)
{
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (long j=0;j<16;j++)
		{
			diff_prev_buf[j].u = vec_set1_epi64(0);
		}

		uint64_t cur_buf_pos = 0;
		for (i=0;i<num_rf_vals_unique_multiple;i+=vec_len_pd)
		{
			union {
				vec_d_t d;
				vec_i_t u;
			} diff, diff_prev, diff_ord2;
			diff.u = vec_set1_epi64(0);
			diff_prev.u = vec_set1_epi64(0);
			vec_i_t len;
			vec_i_t row_rel, col_rel;

			len = vec_set_iter_epi64(iter, data_val_lens[i+iter]);

			// vec_i_t best_buf_pos = (len >> 4ULL);
			vec_i_t best_buf_pos = vec_srli_epi64(len, 4ULL);
			// len &= 15ULL;
			...
			diff_prev_buf[cur_buf_pos].u = vec_set1_epi64(0);
			for (long j=0;j<vec_len_pd;j++)
				diff_prev.u[j] = diff_prev_buf[best_buf_pos[j]].u[j];

			// diff.u = diff_prev.u + diff_ord2.u;
			diff.u = vec_add_epi64(diff_prev.u, diff_ord2.u);
			// val.u = val.u + diff.u;
			val.u = vec_add_epi64(val.u, diff.u);


			// Store current diff.
			diff_prev_buf[cur_buf_pos].u = diff.u;
			cur_buf_pos = (cur_buf_pos + 1) & 15ULL;
		...
		for (i=num_rf_vals_unique_multiple;i<num_rf_vals_unique;i++)
		{
			...
			uint64_t best_buf_pos = (len >> 4ULL);
			len &= 15ULL;
			...
			cur_buf_pos = (i / vec_len_pd) & 15ULL;
			diff_prev_buf[cur_buf_pos].u[lane_id] = 0;
			diff_prev.u = diff_prev_buf[best_buf_pos].u[lane_id];

			diff.u = diff_prev.u + diff_ord2.u;
			val.u[lane_id] = val.u[lane_id] + diff.u;

			// Store current diff.
			diff_prev_buf[cur_buf_pos].u[lane_id] = diff.u;
