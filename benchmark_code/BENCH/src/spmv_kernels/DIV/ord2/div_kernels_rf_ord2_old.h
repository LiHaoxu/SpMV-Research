	...
	*((uint32_t *) &data_intro[data_intro_bytes]) = num_rfs;
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique;i++)
		{
			...
			long buf_pos = (i / vec_len_pd) % 16;
			diff_prev_buf[lane_id][buf_pos].u = 0;
			uint64_t max_pos = buf_pos;
			uint64_t max_diff_ord2 = diff.u - diff_prev_buf[lane_id][max_pos].u;
			uint64_t max_sb = __builtin_clzl(max_diff_ord2);
			for (long j=0;j<16;j++)
			{
				uint64_t dd = diff.u - diff_prev_buf[lane_id][j].u;
				uint64_t sb = __builtin_clzl(dd);
				if (((int64_t) dd > 0) && sb > max_sb)
				// if (sb > max_sb)
				{
					max_pos = j;
					max_diff_ord2 = dd;
			...
			diff_prev_buf[lane_id][buf_pos].u = diff.u;

			if (i_s == 0 && lane_id == 0)
				printf("diff=%064lb diff_ord2=%064lb diff_ord2_best16=%064lb %3d %3d %3d , %3d %3d , tz=%3d\n", diff.u, diff_ord2.u, max_diff_ord2, __builtin_clzl(diff.u), __builtin_clzl(diff_ord2.u), (int) max_sb,
						__builtin_clzl(diff_ord2.u) - __builtin_clzl(diff.u), ((int) max_sb) - __builtin_clzl(diff.u), __builtin_ctzl(diff.u));
			...
			else
			{
				...
				if (diff_ord2.u != buf_diff)
					error("diff_ord2.u != buf_diff : %064lb != %064lb\n"
					      "leading_sign_bits=%ld, trailing_zeros=%ld, rem_tz=%ld, len_bits=%ld, len=%ld",
					      diff_ord2.u, buf_diff,
					      leading_sign_bits, trailing_zeros, rem_tz, len_bits, len);
			}
		}
