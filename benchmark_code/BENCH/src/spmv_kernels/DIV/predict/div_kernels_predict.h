	...
	for (i=0;i<num_vals_padded;i++)
	{
		...
		else
		{
...

			{
				...
				vec_i_t v_sign;
				v_diff.u = vec_set1_epi64(diff.u);
				vec_i_t v_len_bits;
				v_len_bits = vec_set1_epi64(len_bits);
				vec_i_t v_shift;
				v_shift = vec_set1_epi64(tz);


				// vec_i_t v_pow2 = (1ULL << v_len_bits);
				vec_i_t v_pow2 = vec_sllv_epi64(vec_set1_epi64(1ULL), v_len_bits);

				// vec_i_t v_msb = v_pow2 >> 1ULL;
				vec_i_t v_msb = vec_srli_epi64(v_pow2, 1ULL);
				// v_sign = ~((v_diff.u & v_msb) - 1ULL);
				v_sign = vec_and_si(v_diff.u, v_msb);
				// sign = -sign;
				v_sign = vec_sub_epi64(vec_set1_epi64(0), v_sign);   // There is no negation, so we have to do it via subtracting from 0.

				// vec_i_t v_mask = v_pow2 - 1ULL;
				vec_i_t v_mask = vec_sub_epi64(v_pow2, vec_set1_epi64(1ULL));
				// v_diff.u &= v_mask;
				v_diff.u = vec_and_si(v_diff.u, v_mask);

				// v_diff.u = v_diff.u | v_sign;   // AFTER v_mask, BEFORE v_shift.
				v_diff.u = vec_or_si(v_diff.u, v_sign);

				// v_diff.u <<= v_shift;
				v_diff.u = vec_sllv_epi64(v_diff.u, v_shift);
...
decompress_and_compute_kernel_sort_diff_base(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate,
		void (* gather_coords)(long i, unsigned char * data_coords, const uint64_t coords_bytes, uint64_t row_bits, uint64_t col_bits, vec_i_t * row_rel_out, vec_i_t * col_rel_out)
		)
{
	int tnum = omp_get_thread_num();
	ValueType * x_rel;
	...
	// vec_d_t lane_run_avg = vec_set1_pd(1.0);
	vec_d_t lane_run_avg = vec_set1_pd(0.0);
	val.u = vec_set1_epi64(0ULL);
	for (i=0;i<num_vals_padded;i+=4)
	{
		val_prev.u = val.u;

		// val.d = val.d * ratios;
		// val.d = vec_mul_pd(val.d, ratios);
		// val.d = vec_add_pd(val.d, ratios);
		...
		// len_bits = len << 3ULL;
		len_bits = vec_slli_epi64(len, 3ULL);
		...
		*/
		...
		// vec_i_t sign = diff.u & msb;
		vec_i_t sign = vec_and_si(diff.u, msb);
		// sign = -sign;
...
decompress_and_compute_kernel_sort_diff_select(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate)
{
	...
	// if (row_bits == 8)
	{
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r1_c1_v);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r1_c2_v);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r1_c3_v);
			default:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r1_c4_v);
		}
	}
	...
	if (row_bits == 0)
	{
		...
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c1_v);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c2_v);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c3_v);
			default:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c4_v);
		}
	...
	switch (coords_bytes) {
		case 1:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_dense_1_v);
		case 2:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_dense_2_v);
		case 3:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_dense_3_v);
		case 4:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_dense_4_v);
	}

	return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_dense_v);
