		...
		time_compress = time_it(1,
			...
			_Pragma("omp parallel")
			{
				...
				while (j < j_e)
				{
					...
					if (size > 0)
					{
						pos = dynarray_push_back_array_uc(da_compr_data, packet_buf, size);
						dynarray_push_back_l(da_packet_data_offsets, pos);
						dynarray_push_back_l(da_packet_nnz_offsets, num_vals);
						t_num_packets++;
					}
				...
				_Pragma("omp single")
				{
					packet_data_offsets = (typeof(packet_data_offsets)) aligned_alloc(64, num_packets *  sizeof(*packet_data_offsets));
					packet_nnz_offsets = (typeof(packet_nnz_offsets)) aligned_alloc(64, num_packets * sizeof(*packet_nnz_offsets));
					compr_data = (typeof(compr_data)) aligned_alloc(64, compr_data_size);
				}
