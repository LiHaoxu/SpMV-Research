...
// __global__
void
compute_div_device(__attribute__((unused)) DIVArray * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	#pragma omp parallel
	{
		long tnum = omp_get_thread_num();
...
void
compute_div(__attribute__((unused)) DIVArray * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	compute_div_device(csr, x, y);
}
