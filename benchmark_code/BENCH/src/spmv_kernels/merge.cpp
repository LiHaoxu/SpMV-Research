...
csr_to_format(INT_T * row_ptr, INT_T * col_ind, ValueTypeReference * values, long m, long n, long nnz, long symmetric, long symmetry_expanded)
{
	...
	merge->row_ptr = (typeof(merge->row_ptr)) aligned_alloc(64, (m+1) * sizeof(*merge->row_ptr));
	merge->ja = (typeof(merge->ja)) aligned_alloc(64, nnz * sizeof(*merge->ja));
	merge->a = (typeof(merge->a)) aligned_alloc(64, nnz * sizeof(*merge->a));
	#pragma omp parallel for
	for (long i=0;i<m+1;i++)
		merge->row_ptr[i] = row_ptr[i];
	#pragma omp parallel for
	for(long i=0;i<nnz;i++)
	{
		merge->a[i]=values[i];
		merge->ja[i]=col_ind[i];
	}
