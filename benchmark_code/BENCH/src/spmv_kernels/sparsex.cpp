...
struct CSXArrays : Matrix_Format
{
	...
	CSXArrays(long m, long n, long nnz) : Matrix_Format(m, n, nnz)
	{
...
csr_to_format(INT_T * row_ptr, INT_T * col_ind, ValueTypeReference * values, long m, long n, long nnz, long symmetric, long symmetry_expanded)
{
	...
	struct CSXArrays * csx = new CSXArrays(m, n, nnz);
	...
	csx->row_ptr = (typeof(csx->row_ptr)) aligned_alloc(64, (m+1) * sizeof(*csx->row_ptr));
	csx->ja = (typeof(csx->ja)) aligned_alloc(64, nnz * sizeof(*csx->ja));
	csx->a = (typeof(csx->a)) aligned_alloc(64, nnz * sizeof(*csx->a));
	#pragma omp parallel for
	for (long i=0;i<m+1;i++)
		csx->row_ptr[i] = row_ptr[i];
	#pragma omp parallel for
	...
	time = time_it(1,
		input = spx_input_load_csr(csx->row_ptr, csx->ja, csx->a, m, n);
