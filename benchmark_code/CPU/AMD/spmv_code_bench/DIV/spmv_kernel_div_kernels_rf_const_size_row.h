...
extern "C"{
	...
	#include "spmv_kernel_div_gather_coords_const_size_row.h"
#ifdef __cplusplus
}
#endif
	...
	if (ret == 0)
	{
		if (ret == 0)
		{
			...
			if (ret == 0)
			{
				...
				if (ret == 0)
				{
					uint64_t ha = xorshift64_int(ca, 0, 0);
					uint64_t hb = xorshift64_int(cb, 0, 0);
					ret = (ha > hb) ? 1 : (ha < hb) ? -1 : 0;
				}
			}
		}
	}
...
compress_kernel_sort_diff(INT_T * row_ptr, INT_T * ja, ValueType * vals, long i_s, __attribute__((unused)) long i_t_s, __attribute__((unused)) long i_t_e, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	for (i=0;i<max_rf+1;i++)
	{
		...
		/* definition : permutation_interleave(A_in, A_out, N, num_segments, unit_size, solid_units_main, solid_units_rem) */
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		// printf("rf=%3ld , num_rf_vals_unique=%4ld , num_vals=%4ld\n", rf, num_rf_vals_unique, rf * num_rf_vals_unique);
		for (long iter=0;iter<VEC_LEN;iter++)
			val_prev[iter].u = 0;
...
void
mult_add_serial(ValueType * x_rel, ValueType * y_rel, vec_t(VTF, VEC_LEN) val, vec_t(i32, VEC_LEN) row_rel, vec_t(i32, VEC_LEN) col_rel)
{
	for (long iter=0;iter<VEC_LEN;iter++)
		y_rel[vec_array(i32, VEC_LEN, row_rel)[iter]] += vec_array(VTF, VEC_LEN, val)[iter] * x_rel[vec_array(i32, VEC_LEN, col_rel)[iter]];
}
...
decompress_and_compute_kernel_sort_diff_base(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate,
		void (* gather_coords)(long i, unsigned char * data_coords, const uint64_t coords_bytes, uint64_t row_bits, uint64_t col_bits, uint32_t * row_rel_out, uint32_t * col_rel_out),
		void (* gather_coords_v)(long i, unsigned char * data_coords, const uint64_t coords_bytes, uint64_t row_bits, uint64_t col_bits, vec_t(i32, VEC_LEN) * row_rel_out, vec_t(i32, VEC_LEN) * col_rel_out)
		)
{
	int tnum = omp_get_thread_num();
	struct packet_header * header;
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique_multiple;i+=VEC_LEN)
		{
			...
			vec_t(i32, VEC_LEN) row_rel, col_rel;
			...
			for (j=0;j<rf;j++)
			{
				...
				if (validate)
				{
					...
					vec_t(i32, VEC_LEN) row, col;
					row = vec_add(i32, VEC_LEN, row_rel, vec_set1(i32, VEC_LEN, row_min));
					col = vec_add(i32, VEC_LEN, col_rel, vec_set1(i32, VEC_LEN, col_min));
					for (r=0;r<VEC_LEN;r++)
					{
						window_ia[k+r] = vec_array(i32, VEC_LEN, row)[r];
						window_ja[k+r] = vec_array(i32, VEC_LEN, col)[r];
					}
				}
				else
				{
		...
		for (i=num_rf_vals_unique_multiple;i<num_rf_vals_unique;i++)
		{
			...
			for (j=0;j<rf_multiple;j+=VEC_LEN)
			{
				...
				// if (rf == 217)
				// {
					// for (long w=0;w<VEC_LEN;w++)
					// {
						// printf("lane=%ld, row=%d, col=%d\n", w, vec_array(i32, VEC_LEN, row_rel)[w], vec_array(i32, VEC_LEN, col_rel)[w]);
					// }
				// }
				if (validate)
				{
