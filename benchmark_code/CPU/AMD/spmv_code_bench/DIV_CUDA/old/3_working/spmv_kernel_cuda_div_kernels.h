...
extern "C"{
	...
	#include "spmv_kernel_cuda_div_gather_coords.h"
#ifdef __cplusplus
}
#endif
...
compress_kernel_div(INT_T * row_ptr, INT_T * ja, ValueTypeReference * vals, __attribute__((unused)) long symmetric, long i_s, __attribute__((unused)) long i_t_s, __attribute__((unused)) long i_t_e, long j_s,
		__attribute__((unused)) unsigned char * buf, long num_vals, long * num_vals_out, long * size_out)
{
	int tnum = omp_get_thread_num();
	struct thread_data * td = tds[tnum];
		...
		)
{
	...
	row_bits = buf[offsetof(struct packet_header, row_bits)];
	col_bits = buf[offsetof(struct packet_header, col_bits)];
	num_vals = read_int32_t(&buf[offsetof(struct packet_header, num_vals)]);
	num_rows = read_int32_t(&buf[offsetof(struct packet_header, num_rows)]);
	row_min = read_int32_t(&buf[offsetof(struct packet_header, row_min)]);
	col_min = read_int32_t(&buf[offsetof(struct packet_header, col_min)]);
	...
	if (tid > 0)
		data_val_lanes += read_int32_t((unsigned char *) &data_val_lanes_offsets[tid-1]);
	data_val_lanes_bytes = read_int32_t((unsigned char *) &data_val_lanes_offsets[VEC_LEN-1]);
...
decompress_and_compute_kernel_div_select(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, INT_T * ia_out, INT_T * ja_out, ValueType * a_out, const int validate)
{
	...
	if (row_bits == 0)
	{
		switch (col_bits) {
			...
			case 24:
				return decompress_and_compute_kernel_div_base(buf, x, y, ia_out, ja_out, a_out, validate, gather_coords_sparse_r0_c3);
			case 32:
	...
	switch (coords_bytes) {
		case 1:
			return decompress_and_compute_kernel_div_base(buf, x, y, ia_out, ja_out, a_out, validate, gather_coords_dense_1);
		case 2:
			return decompress_and_compute_kernel_div_base(buf, x, y, ia_out, ja_out, a_out, validate, gather_coords_dense_2);
		case 3:
			return decompress_and_compute_kernel_div_base(buf, x, y, ia_out, ja_out, a_out, validate, gather_coords_dense_3);
		case 4:
			return decompress_and_compute_kernel_div_base(buf, x, y, ia_out, ja_out, a_out, validate, gather_coords_dense_4);
	}
...
decompress_kernel_div(INT_T * ia_out, INT_T * ja_out, ValueType * a_out, unsigned char * restrict compr_data_d, long * packet_data_offsets_d, long * packet_nnz_offsets_d)
{
	...
	if (threadIdx.x == 0)
	{
		long num_vals = read_int32_t(&compr_data_d[offsetof(struct packet_header, num_vals)]);
