...
struct CSRArrays : Matrix_Format
{
	...
	CSRArrays(INT_T * ia, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), ia(ia), ja(ja), a(a)
	{
		...
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(startEvent_memcpy_thread_i_s, stream));
		...
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(endEvent_memcpy_thread_i_s, stream));

		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(startEvent_memcpy_thread_i_e, stream));
		...
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(endEvent_memcpy_thread_i_e, stream));
...
compute_csr(CSRArrays * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	...
	// 	for(int k=0;k<num_loops;k++){
	gpu_kernel_csr_basic<<<grid_dims, block_dims, 0, csr->stream>>>(thread_i_s_d, thread_i_e_d, csr->ia_d, csr->ja_d, csr->a_d, csr->x_d, csr->y_d);
	gpuCudaErrorCheck(cudaPeekAtLastError());
	gpuCudaErrorCheck(cudaDeviceSynchronize());
	// 	}
	// );
