...
struct COOArrays : Matrix_Format
{
	...
	COOArrays(INT_T * ia, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), colind(ja), a(a)
	{
		...
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(startEvent_memcpy_rowind, stream));
		gpuCudaErrorCheck(cudaMemcpyAsync(rowind_d, rowind_h, nnz * sizeof(*rowind_d), cudaMemcpyHostToDevice, stream));
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(endEvent_memcpy_rowind, stream));

		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(startEvent_memcpy_colind, stream));
		gpuCudaErrorCheck(cudaMemcpyAsync(colind_d, colind_h, nnz * sizeof(*colind_d), cudaMemcpyHostToDevice, stream));
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(endEvent_memcpy_colind, stream));
		...
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(startEvent_create_matA, stream));
		...
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(endEvent_create_matA, stream));
...
compute_coo(COOArrays * restrict coo, ValueType * restrict x, ValueType * restrict y)
{
	...
	if (coo->x == NULL)
	{
		...
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(coo->startEvent_create_vecX, coo->stream));
		gpuCusparseErrorCheck(cusparseCreateDnVec(&coo->vecX, coo->n, coo->x_d, ValueTypeCuda));
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(coo->endEvent_create_vecX, coo->stream));

		// Create dense vector y
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(coo->startEvent_create_vecY, coo->stream));
		gpuCusparseErrorCheck(cusparseCreateDnVec(&coo->vecY, coo->m, coo->y_d, ValueTypeCuda));
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(coo->endEvent_create_vecY, coo->stream));

		// Allocate an external buffer if needed
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(coo->startEvent_spmv_buffersize, coo->stream));
