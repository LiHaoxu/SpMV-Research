...
compute_csr(CSRArrays * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	...
	if (csr->x == NULL)
	{
		...
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(csr->startEvent_create_vecX, csr->stream));
		gpuCusparseErrorCheck(cusparseCreateDnVec(&csr->vecX, csr->n, csr->x_d, ValueTypeCuda));
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(csr->endEvent_create_vecX, csr->stream));

		// Create dense vector y
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(csr->startEvent_create_vecY, csr->stream));
		gpuCusparseErrorCheck(cusparseCreateDnVec(&csr->vecY, csr->m, csr->y_d, ValueTypeCuda));
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(csr->endEvent_create_vecY, csr->stream));

		// Allocate an external buffer if needed
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(csr->startEvent_spmv_buffersize, csr->stream));
		...
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(csr->endEvent_spmv_buffersize, csr->stream));
		// printf("SpMV_bufferSize = %zu bytes\n", csr->bufferSize, csr->bufferSize); // size of the workspace that is needed by cusparseSpMV()

		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(csr->startEvent_spmv_preprocess, csr->stream));
		...
		if(TIME_IT) gpuCudaErrorCheck(cudaEventRecord(csr->endEvent_spmv_preprocess, csr->stream));
