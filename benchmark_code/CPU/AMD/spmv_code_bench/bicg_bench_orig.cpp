		...
		long m, __attribute__((unused)) long n, __attribute__((unused)) long nnz, ValueType * b, ValueType * x_res_out, long max_iterations)
{
	...
	y = (typeof(y)) malloc(m * sizeof(*y));
	t = (typeof(t)) malloc(m * sizeof(*t));
	buf1 = (typeof(buf1)) malloc(m * sizeof(*buf1));
	buf2 = (typeof(buf2)) malloc(m * sizeof(*buf2));
	...
	while (k < max_iterations)
	{
		...
		if ((k > 0) && !(k % restart_k))
		{
			...
			#pragma omp parallel
			{
				vector_pw_add(rk_explicit, b, -1, buf1, m);
		...
		#pragma omp parallel
		{
			...
			tmp1 = vector_dot(buf1, buf2, m);
			tmp2 = vector_dot(buf1, buf1, m);
			s_w = tmp1 / tmp2;
			...
			// pk = rk + s_b * (pk - s_w * v)
			vector_pw_add(buf1, pk, -s_w, v, m);
			vector_pw_add(pk, rk, s_b, buf1, m);
