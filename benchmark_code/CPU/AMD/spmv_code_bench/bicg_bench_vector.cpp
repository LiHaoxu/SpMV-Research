...
vector_dot(ValueType * x1, ValueType * x2, long N)
{
	ValueType total = 0;
	ValueType partial = 0;
	vec_d_t v_partial = vec_set1_pd(0);
	long N_multiple;
	long i;
	N_multiple = (N/vec_len_pd) * vec_len_pd;
	#pragma omp for
	for (i=0;i<N_multiple;i+=vec_len_pd)
	{
		v_partial = vec_fmadd_pd(vec_loadu_pd(&x1[i]), vec_loadu_pd(&x2[i]), v_partial);
		// v_partial = vec_add_pd(v_partial, vec_mul_pd(vec_loadu_pd(&x1[i]), vec_loadu_pd(&x2[i])));
	}
		...
		long m, __attribute__((unused)) long n, __attribute__((unused)) long nnz, ValueType * b, ValueType * x_res_out, long max_iterations)
{
	...
	__attribute__((cleanup(cleanup_free))) ValueType * buf = NULL;
	...
	// Store the inverse, so that we can replace divisions with multiplications.
	...
	while (k < max_iterations)
	{
		...
		#pragma omp parallel
		{
			...
			// s_w = (inv(K1) * t, inv(K1) * s) / (inv(K1) * t, inv(K1) * t)
			{
				...
				vec_d_t v_partial_1 = vec_set1_pd(0), v_partial_2 = vec_set1_pd(0);
				ValueType val_1, val_2;
				vec_d_t v_val_1, v_val_2;
				long m_multiple;
				m_multiple = (m/vec_len_pd) * vec_len_pd;
				#pragma omp for
				for (i=0;i<m_multiple;i+=vec_len_pd)
				{
					v_val_1 = vec_mul_pd(vec_loadu_pd(&t[i]), vec_loadu_pd(&K[i]));
					v_val_2 = vec_mul_pd(vec_loadu_pd(&s[i]), vec_loadu_pd(&K[i]));
					v_partial_1 = vec_fmadd_pd(v_val_1, v_val_2, v_partial_1);
					v_partial_2 = vec_fmadd_pd(v_val_1, v_val_1, v_partial_2);
				}
				for (i=0;i<vec_len_pd;i++)
				{
					partial_1 += v_partial_1[i];
					partial_2 += v_partial_2[i];
				}
