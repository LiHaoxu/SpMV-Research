...
if [[ ${ARCH} == x86_64 ]]; then
    ...
    # common EPI compiler flags for all machines
    ...
    elif [[ "${MACHINE_NAME}" == *"pioneer"* ]]; then
        # rvv-0.7 - Pioneer
        CFLAGS+=" -mllvm -vectorizer-use-vp-strided-load-store"
        CFLAGS+=" -mllvm -enable-mem-access-versioning=0"
    fi
fi
...
export LEVEL3_CACHE_NUM_NODES=0 # "$(( NUM_CPUS / LEVEL3_CACHE_CPUS_PER_NODE ))"
